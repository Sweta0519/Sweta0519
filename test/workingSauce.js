...
describe('Working Sauce', function () {
    ...
    let enterSauce = await driver.findElement(By.id("i_am_a_textbox"));
    await enterSauce.clear();
    await enterSauce.sendKeys("Sauce");

    // Task III
    let email = await driver.findElement(By.id("fbemail"));
    await email.sendKeys("sweta1234");

    let comment = await driver.findElement(By.id("comments"));
    await comment.sendKeys("Hi");

    let send = await driver.findElement(By.id("submit"));
    await send.click();

    // Bonus task refered from https://wiki.saucelabs.com/display/DOCS/Test+Configuration+Options
